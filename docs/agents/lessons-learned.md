...
- For workflow-backed chat, keep `chats.workflowRunId` as the long-lived durable run pointer and `chats.activeStreamId` as the current turn's namespaced resumable stream id; reusing one field for both causes broken reconnect semantics or duplicate history because canonical chat messages already come from Postgres.
- When upgrading `workflow` for DurableAgent chat, keep `package.json`, `bun.lock`, and the installed runtime in sync before testing. A partial upgrade can leave local runs stamped with an older `workflowCoreVersion`, which then fails internal AI steps like `step//@workflow/ai...//doStreamStep` with `Step not found` until the lockfile is regenerated and the dev server is restarted.
- In workflow-backed chat startup, publish the turn stream namespace before expensive sandbox hydration and do not overwrite `activeStreamId` back to `null` in the route after `start(...)`; otherwise the first send can 504 waiting for a stream that the workflow has not advertised yet, or that the route just erased.
- In durable-agent workflow tools, any sandbox reconnect or skill discovery that touches `@open-harness/sandbox` must happen inside a `"use step"` function, not directly in a tool `execute` callback; one stray reconnect in `skill.execute` was enough to make the workflow compiler traverse `packages/sandbox/*` and fail with node-module-in-workflow errors.
...
- For the web chat product, using `DurableAgent` does not imply a long-lived workflow per chat session; keep chat execution as one workflow run per user request, and let each request return its own `turn-1` stream. A hook-paused multi-turn workflow changes the product lifecycle and creates unnecessary `waiting`/resume complexity.
