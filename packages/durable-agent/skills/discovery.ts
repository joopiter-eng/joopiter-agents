import { basenamePath, joinPath } from "../path-utils";
import {
...
interface SkillDiscoveryDirent {
  name: string;
  isDirectory(): boolean;
}

interface SkillDiscoverySandbox {
  access(path: string): Promise<void>;
  stat(path: string): Promise<{ isDirectory(): boolean }>;
  readdir(
    path: string,
    options: { withFileTypes: true },
  ): Promise<SkillDiscoveryDirent[]>;
...
async function findSkillFile(
  sandbox: SkillDiscoverySandbox,
  skillDir: string,
): Promise<string | null> {
  const uppercasePath = joinPath(skillDir, "SKILL.md");
  const lowercasePath = joinPath(skillDir, "skill.md");
...
export async function discoverSkills(
  ...
  for (const dir of directories) {
    ...
    for (const entry of entries) {
      ...
      const skillDir = joinPath(dir, entry.name);
      ...
      skills.push({
        ...
        path: skillDir,
        filename: basenamePath(skillFile),
